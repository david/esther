...
| Topic | Value |
|---|---|
| Recommendation | Add a strict core runtime guard: when a command observes a DCB tag boundary, the emitted event must include every observed tag. |
...
| Verification focus | Matching tags, missing observed tag, extra event tags, and no append/fanout on failure. |

## Decisions Needed

None. User approved strict runtime error with no opt-out.

## Changed Since Last Draft

Removed opt-out from approved design. Guard is strict; mismatches must be fixed by emitted tags or observed boundary choice.
...
This extends existing DCB ownership in `src/core/pipeline.ts` and leaves event stores responsible only for atomic append precondition checks. No opt-out is included; mismatches must be fixed by changing emitted tags or by choosing a different observed boundary.
...
Given `castTagQuery(...)` resolves a subject, reads `tags(subject)`, and records boundary tags, the emitted event must include those tags.
...
| Advanced read/write tag mismatch | app command modeling only | command design | unclear owner | medium | remodel tags/read boundary; no opt-out in this feature |
...
7. compare observed boundary tags to parsed event tags
...
| Command runtime | can append mismatched event | rejects mismatch | yes | none | normal |
...
- Do not add an opt-out or bypass in this implementation; revisit in a separate issue only with a concrete valid use case.
...
4. Docs/examples
   ...
   - `llms.txt` explains mismatch error and that there is no opt-out
