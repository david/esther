...
- No skipping full repo verification because the implementation is docs-only.
...
- `plan/01-implementation-plan.md` — superseded plan.
- `plan/checks/01-plan-sanity.md` — revision request: full repo gates are mandatory; targeted searches are only extra checks.
...
- `doc/testing.md` and `doc/commands.md` — full-repo verification expectations.
...
|---|---|---|---|---|
| Documentation correctness | full repo gates plus focused `llms.txt` review | stale examples possible | verify stale tokens are gone, current APIs appear, and full repo checks pass | developers, LLM consumers |
...
No new runtime tests are required unless implementation discovers code examples are compiled or extracted by tooling. Full repo gates remain mandatory despite docs-only scope.

Required focused documentation checks:

1. Stale API search:
   ```bash
   rg -n "createFastifyAdapter|projectors:|processors:" llms.txt
   rg -n "tagQuery|castTagQuery|eventsByTagsDescriptor|queryByTags|schemas:|fold:" llms.txt
   ```
   Expected: no stale public examples. If `schemas:` appears only inside `defineReducer`, confirm it is not raw `tagQuery` / `castTagQuery` config. Also inspect positional `eventsByTagsDescriptor(...)` and `eventStore.queryByTags(...)` snippets so raw reducer inputs do not survive without object-literal `schemas:` / `fold:` tokens.
...
3. Snippet/source cross-check:
   - compare representative examples against `src/__tests__/type-check.ts` and current public exports in `src/index.ts` / `src/adapters/fastify/index.ts`.

Required full repo gates after docs edit:
...
If any full gate fails due to pre-existing repository state or environment limits, implementation checkpoint must record:

- exact command run
- concise failure-output summary
- evidence that failure is not caused by the `llms.txt` edit
- suggested owner / next step

Targeted searches never substitute for full gates.
...
- Confirm removed APIs are either absent or explicitly marked compatibility-only:
  ...
  - no raw positional reducer inputs in `eventsByTagsDescriptor(...)` / `eventStore.queryByTags(...)`
...
- Confirm verification checkpoint records focused searches plus full `typecheck`, `lint`, and `test` results.

## Rollout / deploy notes

Docs-only change. No package version, migration, backfill, rebuild, or deploy ordering required. Ship with normal PR/review flow after plan check, breakdown, implementation, full gates, and QA artifacts.
...
| docs-only status tempts skipped gates | broken windows or hidden repo drift | require `bun run typecheck`, `bun run lint`, and `bun run test`; record any pre-existing/environment failures with evidence |
...
- Focused stale/current API searches are run and recorded in implementation checkpoint.
- Full repo gates are run and recorded in implementation checkpoint:
  - `bun run typecheck`
  - `bun run lint`
  - `bun run test`
- Any gate failure is recorded with exact command, failure summary, and evidence whether it is pre-existing/environmental versus caused by the `llms.txt` edit.
...
- Watch items for implementation checkpoints and drift checks:
  ...
  - ensure positional `eventsByTagsDescriptor(...)` and `eventStore.queryByTags(...)` examples are reducer-backed
